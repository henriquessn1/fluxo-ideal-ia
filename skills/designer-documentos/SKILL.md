...
## Política de acesso do documento (`acesso` + `visivel_por`)

**Quem pode VER um documento gerado é DADO TIPADO no modelo** — dois campos que você seta ao
criar/editar o template, não texto solto nem metadado adivinhado. É a **fonte da verdade** de
visibilidade (substitui o antigo jsonb `metadados`). Setar isso certo é parte do design do documento.

**Os dois campos:**

- **`acesso`** — um **enum** com exatamente **três** valores:
  - **`publico`** — **todos** enxergam o documento. Use para material que não é sigiloso
    (folheto, material educativo, orientação geral ao paciente).
  - **`restrito`** — só quem tem uma das **roles listadas em `visivel_por`** enxerga. É o caso
    da maioria dos documentos clínicos (receita, atestado, laudo).
  - **`privado`** — **só o autor** (quem criou o documento / o profissional a ele atribuído).
    Use para nota pessoal / rascunho do profissional que ninguém mais deve ver.
- **`visivel_por`** — uma **lista de roles** (ex.: `["medico","atendente","cliente"]`). **Só faz
  efeito quando `acesso=restrito`.** Em `publico` e `privado` o campo é **IGNORADO** (o sistema
  não rejeita se vier preenchido — simplesmente não olha). Ou seja: só mexa em `visivel_por`
  quando `acesso` for `restrito`.

**Default (quando você não seta nada):** `acesso=restrito` + `visivel_por=["medico","atendente","cliente"]`.
Isto é: por padrão, o documento é restrito e visível para médico, atendente e cliente.

**⚠️ Você (a LLM) NÃO seta o autor.** Autor, data de emissão e assinatura são **estampados
AUTOMATICAMENTE pelo serviço** na geração — o "autor" de que trata `privado` é resolvido pela
plataforma (quem criou / o profissional atribuído). Nunca tente passar um campo de autor; foque só
em `acesso` e (quando `restrito`) `visivel_por`.

**⚠️ `visivel_por` só aceita roles VÁLIDAS.** Os nomes têm que existir no **catálogo de roles do
sistema**. Uma role inexistente (ex.: `financeiro` quando essa role não existe na clínica) faz a
criação/edição **falhar** com "roles inexistentes em visivel_por". Na dúvida, use as roles
conhecidas do sistema (ex.: `medico`, `atendente`, `cliente`) — não invente.

### Exemplos por cenário

- **Receita / atestado / laudo** (documento clínico normal — restrito à equipe + o paciente):
  ```json
  { "acesso": "restrito", "visivel_por": ["medico", "atendente", "cliente"] }
  ```
- **Laudo só para a equipe clínica** (o paciente **não** deve ver de imediato):
  ```json
  { "acesso": "restrito", "visivel_por": ["medico", "atendente"] }
  ```
- **Material educativo / folheto** (não sigiloso, todos podem ver):
  ```json
  { "acesso": "publico" }
  ```
- **Nota pessoal do profissional** (só o autor enxerga):
  ```json
  { "acesso": "privado" }
  ```
  > Nos casos `publico` e `privado`, **não** preencha `visivel_por` — ele seria ignorado.
...
- **Acesso é dado tipado (`acesso` + `visivel_por`)** — `publico` (todos) · `restrito` (roles em
  `visivel_por`) · `privado` (só o autor). `visivel_por` só vale em `restrito` (ignorado nos demais).
  Default: `restrito` + `["medico","atendente","cliente"]`. A LLM **não** seta autor (é automático);
  `visivel_por` só aceita **roles válidas** do catálogo do sistema.
